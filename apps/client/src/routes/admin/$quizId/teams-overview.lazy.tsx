...
import { GET_ADMIN_TEAMS_ANSWERS_API_KEY, getAdminTeamsAnswers } from '@/baas/team-answers/get-admin-teams-answers';
...
function TeasOverview() {
  ...
  return (
    <CenterWrapper>
      ...
      <Form {...form}>
        <form className="space-y-4" onSubmit={form.handleSubmit(handleSubmitResults)}>
          <Accordion className="w-full" collapsible type="single">
            {Boolean(data) && Object.keys(data!).map((key) => (
          ...
          <Button className="self-start" type="submit">Submit results</Button>
        </form>
      </Form>
    </CenterWrapper>
  )
...
function TeamFormAnswers({ teamName, form }: { teamName: string, form: UseFormReturn<AnswerSchemaType> }) {
  ...
  return fields.map((field, index) => (
    <div className="p-3 bg-gray-200 rounded mb-2 space-y-3" key={field.id}>
      <p className="font-semibold text-xl">Q{index + 1}: {field.question.content}
        <i>({field.question.answer})</i></p>
      <div className="text-lg">Team answer: <b>{field.answer}</b></div>
      <div className="space-x-2">
        <FormRadioButtons form={form} name={`${teamName}.${index}.score`} options={radioButtonOptions}/>
      </div>
    </div>
  ))
}
