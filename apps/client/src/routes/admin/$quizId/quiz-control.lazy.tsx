...
import {
  GET_ROUND_QUESTIONS_API_KEY,
  getRoundQuestions
} from '@/baas/question/get-round-questions'
...
function QuizControl() {
  ...
  const [status, setStatus] = useState<QuizStatusStatusOptions | undefined>(
    data?.status
  )
  ...
  const { mutate: mutateCurrenQuestion } = useMutation({
    ...
    onSuccess: ({ newCurrentQuestionId }) =>
      setActiveQuestionId(newCurrentQuestionId)
  })
  ...
  function handleNextQuestion() {
    const currentQuestionIndex =
      data?.questions.findIndex(({ id }) => activeQuestionId === id) ?? -1
  ...
  const getSubmitButtonProps = useMemo(() => {
    if (status === QuizStatusStatusOptions.END_ROUND) {
      return {
        label: 'Check answers',
        handler: () => mutateMoveQuizToEvaluationState(quizId)
      }
    }
    ...
    <CenterWrapper>
      ...
      {status === QuizStatusStatusOptions.PLAYING &&
        data?.questions.map(({ content, answer, id }, index) => (
          <div
            className={cn(
              getActiveQuestionStyles(id),
              'self-start rounded-md bg-gray-100/5 w-full px-4 py-3'
            )}
            key={id}
          >
            <p>
      ...
      {status === QuizStatusStatusOptions.END_ROUND && (
        <h2 className="font-semibold text-2xl">
          Let's give teams a chance to fix their answers one more time
        </h2>
      )}
      <Button onClick={getSubmitButtonProps.handler}>
        {getSubmitButtonProps.label}
      </Button>
    </CenterWrapper>
  )
}
