...
function Waiting() {
  ...
  function handleIncomingUpdate({ status }: { status: QuizStatusStatusOptions }) {
  ...
  return (
    <CenterWrapper className="items-center flex flex-col gap-4">
      ...
      <AlienTaken/>
      <p className="text-xl text-center">Don't worry <b>{teamName}</b>, the quiz will start in a few moments</p>
    </CenterWrapper>
  )
}
