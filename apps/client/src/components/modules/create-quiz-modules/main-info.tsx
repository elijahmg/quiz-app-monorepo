...
export function MainInfo({ onSuccess }: MainInfoProps): React.ReactNode {
  ...
  return (
    <Form {...form}>
      <form className="grid grid-cols-2 gap-6 min-w-[1280px] h-[80%]" onSubmit={form.handleSubmit(onSubmit)}>
        ...
        <div className="space-y-4 max-h-[70%] overflow-y-scroll p-4">
          ...
          <Select onValueChange={(newActiveRound) => {
            ...
            <SelectContent>
              <SelectGroup>
                ...
                {rounds.map((round) => <SelectItem key={round} value={round}>{round}</SelectItem>)}
              </SelectGroup>
            </SelectContent>
          </Select>
          {fields.map((item, key) => (
            <div className={item.round === activeRound ? 'block' : 'hidden'} key={item.id}>
              <FormInput form={form} label="Question" name={`questions.${key}.content`}/>
              <FormInput form={form} label="Answer" name={`questions.${key}.answer`}/>
            </div>
          ))}
          <Button
